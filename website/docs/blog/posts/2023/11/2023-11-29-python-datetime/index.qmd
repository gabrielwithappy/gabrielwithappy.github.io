---
title: Dataframed을 Dict로 생성 및 변환
subtitle: Dataframed을 Dict로 생성 및 변환
description:
    Dataframed을 Dict로 생성 및 변환
categories:
  - Pandas
...
변환 형식은 `orient`에 전달되고 기본값은 `dict` 형식입니다. 다만 index자 추가되는 `{column -> {index->value}}`와 같은 형식을 갖습니다.
``` {python}
