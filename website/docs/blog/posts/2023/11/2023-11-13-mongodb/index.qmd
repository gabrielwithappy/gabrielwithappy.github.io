...
services:
  mongo-db:
    ...
    restart: always
    environment:
      MONGO_INITDB_ROOT_USERNAME: <루트유저 이름>
      MONGO_INITDB_ROOT_PASSWORD: <루트유저 패스워드>
    volumes:
      - /data:/data/db
    ports:
      - 27017:27017
  mongoexpress:
    ...
    ports:
      - 8081:8081
    environment:
      - ME_CONFIG_MONGODB_ADMINUSERNAME: <루트유저이름>
      - ME_CONFIG_MONGODB_ADMINPASSWORD: <루트유저 패스워드>
      - ME_CONFIG_MONGODB_URL=mongodb://<루트유저이름>:<루트유저 패스워드>@mongo-db:27017
```
