...
정기적으로 포트폴리오의 MDD를 검토함으로써, 시장 환경 변화에 따른 리스크를 모니터링하고 필요 시 조정할 수 있습니다. 이는 장기적인 포트폴리오 안정성을 유지하는 데 도움을 줍니다.

MDD가 큰 시기에 대비해 일정 부분을 현금으로 보유함으로써 시장 회복 시 더 많은 자본을 투입할 수 있습니다.
...
fig.update_layout(
    margin=dict(l=0, r=0, pad=0), # remove due to mis-aligned range button
    # autosize=True,
    hovermode = 'x' or 'y',
    showlegend=True,
    legend = dict(
